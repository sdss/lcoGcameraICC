...
andor_attrs = {'GetCameraHandle.return_value': [DRV_SUCCESS, 1234],
               ...
               'SetReadMode.return_value': DRV_SUCCESS,
               'SetImage.return_value': DRV_SUCCESS,
...
# We have to put these imports after the mock is defined, so our mocked "andor"
# overrides any actual andor.
...
class TestAndorCam(BaseCamTester.BaseCamTester, unittest.TestCase):
    """Tests for the Andor Ikon camera for LCO."""

    def setUp(self):
        super(TestAndorCam, self).setUp()
        # NOTE: verbose=True to print when each andor function is called via safe_call().
        self.cam = andorcam.AndorCam(verbose=False)
    ...
    def tearDown(self):
        ...
        andor.configure_mock(**andor_attrs)
    ...
    def test_cooler_status_acquiring(self):
        ...
        self.cam.cooler_status()
        andor.GetTemperatureF.assert_called_once_with()
        super(TestAndorCam, self)._cooler_status(
            setpoint=-100, ccdTemp=-50, statusText='NotStabilized')
    ...
    def test_cooler_status_stable(self):
        ...
        andor.GetTemperatureF.assert_called_once_with()
        super(TestAndorCam, self)._cooler_status(
            setpoint=-100, ccdTemp=-100, statusText='Stabilized')
    ...
    def test_set_cooler_on_new_temp(self):
        ...
        super(TestAndorCam, self)._cooler_status(
            setpoint=-40, ccdTemp=-25, statusText='NotStabilized')
    ...
    def test_set_cooler_was_off(self):
        ...
        andor.SetTemperature.assert_called_once_with(temp)
        super(TestAndorCam, self)._cooler_status(
            setpoint=-100, ccdTemp=-10, statusText='NotStabilized')
    ...
    def test_set_cooler_off(self):
        ...
        super(TestAndorCam, self)._cooler_status(
            setpoint=0, ccdTemp=-100, statusText='NotStabilized')

    def test_binning(self):
        binning = 10
        self.cam.binning = binning
        self.assertEqual(self.cam.binning, binning)
        andor.SetReadMode.assert_called_once_with(4)
        andor.SetImage.assert_called_once_with(
            binning, binning, 1, self.cam.width, 1, self.cam.height)
...
if __name__ == '__main__':
    ...
    # suite = unittest.TestLoader().loadTestsFromName('test_Controllers.TestAndorCam.test_cooler_status_off')
    if suite:
